...
const CompetenceCard = ({ title, details, imageSRC, stackIcons, linkDemo, linkGithub, linkProjet, year }) => {
    return (
        <div className='col-md-6 col-sm-7 cardInline'>
            <div className='card card-cover h-100 overflow-hidden rounded-4 shadow' id="backgroundImg" style={{backgroundImage:`url(${imageSRC})`, maxHeight: 700}}>
                <div className='d-flex flex-column h-100 p-5 pb-3 text-shadow'>
                    ...
                    {year ? <div className='overflow-scroll mb-3' id="card-details-scroll">{year}</div> : null}
                    <div className='overflow-scroll mb-3' id="scrollbarOverflow">{details}</div>
                    ...
                    <footer className='card-footer justify-content-evenly d-flex bg-transparent border-top-0 px-0 pt-3'>
                       {linkProjet || linkDemo ? <a className="btn btn-light btn-base text-dark rounded-4 shadow col-auto preface__logoBot"id="lienProject" href={linkDemo ? linkDemo : linkProjet} rel="noopener noreferrer" target='_blank' aria-label={linkDemo ? "lien de démonstration" : "lien vers le projet"} title={linkDemo ? 'Démonstration du site' : 'Lien vers le projet'}>
                            <PlayArrowIcon /> {linkDemo ? 'Demo' : 'Lien vers le projet'}
                        </a> : null}
                        {linkGithub ? <a className="btn btn-light btn-base text-dark rounded-4 shadow col-auto preface__logoBot" href={linkGithub} rel="noopener noreferrer" target='_blank' aria-label='Lien vers le repo Github' title="repo Github"><GitHubIcon /> Github</a> : null}
                    </footer>
                </div>
            </div>
        </div>
